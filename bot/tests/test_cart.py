...
async def test_clear_and_checkout_confirmations_and_success():
    ...
    auth.pay_order.assert_awaited_once_with(confirm.from_user, 9)
    auth.get_my_order.assert_awaited_once_with(confirm.from_user, 9)
    text = confirm.message.edit_text.await_args.args[0]
...
async def test_typed_client_pays_order_with_local_payment_endpoint():
    async def handler(request):
        assert request.url.path == "/api/orders/9/pay/"
    ...
    try:
        result = await client.pay_order(123, 9)
    finally:
        await client.close()
